...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['add'])) {
        ...
        if ($stmt->execute()) { $message = "Admin berhasil ditambahkan."; } else { $message = "Error: " . $stmt->error; }
        $stmt->close();
        ...
        if ($stmt->execute()) { $message = "Admin berhasil diubah."; } else { $message = "Error: " . $stmt->error; }
        $stmt->close();
    } elseif (isset($_POST['delete'])) {
        $id = $_POST['id'];
        $stmt = $conn->prepare("DELETE FROM users WHERE id = ?");
        $stmt->bind_param("i", $id);
        if ($stmt->execute()) { $message = "Admin berhasil dihapus."; } else { $message = "Error: " . $stmt->error; }
        $stmt->close();
    }
}
?>
...
<main class="page-container">
    <div class="container">
        ...
        <div class="admin-form-container">
            ...
            <form action="manage_admin.php" method="POST" class="admin-form">
                ...
                <tbody>
                    ...
                    if ($result->num_rows > 0) {
                        while($row = $result->fetch_assoc()) {
                            ...
                            echo '<form id="delete-form-'.$row['id'].'" action="manage_admin.php" method="POST" style="display:none;">';
                        ...
                        echo '<tr><td colspan="4">Tidak ada akun admin.</td></tr>';
                    }
                    $conn->close();
                    ?>
                </tbody>
